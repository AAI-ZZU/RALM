...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_dir', default='results_with_loss_cvrp-lib', help="Name of results directory")
    ...
    for width in widths:
        for dataset_path in file_names:
            print(dataset_path)
            cost, sl, average_time = eval_dataset("./data/CVRPData/" + dataset_path, width, opts.softmax_temperature, opts)
            cost_list.append(cost)
            average_time_list.append(average_time)
            result = dataset_path[:-4]  # 匹配前两个 `_` 之间的内容
    ...
    res.to_csv(opts.model + "_res_cvrplib.csv", index=False)
    print("success!")