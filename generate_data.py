...
def generate_hcvrp_data(seed,dataset_size, hcvrp_size, veh_num):
    ...
    # speed = rnd.uniform(1, 1, size=(dataset_size, veh_num))
    cap = rnd.randint(20, 41, size=(dataset_size, veh_num))

    # d = np.zeros([dataset_size, hcvrp_size + 1])
    # d = d[:, :-1]  # the demand of depot is 0, which do not need to generate here
    #
    # # vehicle feature
    # # speed = rnd.uniform(1, 1, size=(dataset_size, veh_num))
    # speed = rnd.uniform(0.5, 1, size=(dataset_size, veh_num))
    # cap = np.zeros([dataset_size, veh_num])
...
if __name__ == "__main__":
    ...
    for item in tqdm(dataset):
        filename = os.path.join(datadir, 'v{}_{}_seed{}.pkl'.format( item[0], item[1], seed))

        dataset = generate_hcvrp_data(seed,item[2], item[1], item[0])
