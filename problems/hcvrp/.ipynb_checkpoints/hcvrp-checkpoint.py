...
class HcvrpEnv:
    ...
    def update(self, veh, next_node):
        ...
        if depot_mask.any():
            # 直接使用索引设置掩码
            self.vehicle_node_mask[self.bs_index[depot_mask], veh[depot_mask], :] = False
    ...
    # 注意力掩码 分两种情况：
        # 1， 如果当前车辆货物剩余量 / 满载剩余量<0.95, 那么只有离它最近50个节点才可用有ATTENTION分数，其他的用很小很小的数来代替（注意可用节点数与50的大小，要选择更小的作为实际值）
        # 2,  如果当前车辆货物剩余量 / 满载剩余量>=0.95, 那么那么没有限制
    def generate_attention_mask(self, k=80):
        """
        完全矩阵化注意力掩码生成
        ...
        # 将已访问节点的距离设为正无穷
        distance_matrix[visited_mask] = float(10000.0)

        # 创建低装载率掩码
        low_load_mask = load_ratio <= 0.96
